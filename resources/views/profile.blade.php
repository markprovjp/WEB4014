                                        ...
                                        <div class="text-center mb-3">
                                            @if (Auth::user()->avatar)
                                                <img src="{{ asset('storage/' . Auth::user()->avatar) }}"
                                                    alt="{{ Auth::user()->name }}" class="img-fluid rounded-circle mb-2"
                            ...
                            <div class="list-group">
                                @foreach ($comments as $comment)
                                    <div class="list-group-item list-group-item-action">
                                        ...
                                        <small>
                                            <a href="{{ route('news.detail', $comment->news->id) }}"
                            ...
                            <div class="list-group mb-4">
                                <div class="list-group-item">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" role="switch" id="email_news"
                                            name="notifications[email_news]"
                                            {{ Auth::user()->getNotificationSettingsArray()['email_news'] ?? true ? 'checked' : '' }}>
                                        <label class="form-check-label" for="email_news">Nhận email khi có tin tức
                                            mới</label>
                                    </div>
                                </div>
                                <div class="list-group-item">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" role="switch"
                                            id="email_comments" name="notifications[email_comments]"
                                            {{ Auth::user()->getNotificationSettingsArray()['email_comments'] ?? true ? 'checked' : '' }}>
                                        <label class="form-check-label" for="email_comments">Nhận email khi có người phản
                                            hồi bình luận</label>
                                    </div>
                                </div>
                                <div class="list-group-item">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" role="switch"
                                            id="email_account" name="notifications[email_account]"
                                            {{ Auth::user()->getNotificationSettingsArray()['email_account'] ?? true ? 'checked' : '' }}>
                                        <label class="form-check-label" for="email_account">Nhận email về thay đổi tài
                                            khoản</label>
                                    </div>
                                </div>
                                <div class="list-group-item">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" role="switch"
                                            id="email_marketing" name="notifications[email_marketing]"
                                            {{ Auth::user()->getNotificationSettingsArray()['email_marketing'] ?? false ? 'checked' : '' }}>
                                        <label class="form-check-label" for="email_marketing">Nhận email quảng cáo và
                                            khuyến mãi</label>
                                    </div>
                                </div>
                            </div>
...
@section('scripts')
    ...
    <script>
                ...
                if (this.files && this.files[0]) {
                    ...
                    reader.onload = function(e) {
                        if ($('.profile-avatar').length) {
                            ...
                            $('.avatar-placeholder').replaceWith('<img src="' + e.target.result +
                                '" class="profile-avatar" alt="Avatar preview">');
                        }
                    }
        ...
        // Xử lý hiển thị tab từ PHP
        @if (isset($active_tab) && $active_tab)
            $(document).ready(function() {
                $('.nav-link[href="#{{ $active_tab }}"]').tab('show');
            });
        @endif
    </script>
@endsection
